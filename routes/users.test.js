...
const { commonBeforeAll, commonBeforeEach, commonAfterEach, commonAfterAll, user1Token } = require('./_testCommon');
...
describe('GET /user/:id/account', function() {
	test('works for user', async function() {
		const res = await request(app).get(`/user/33/account`).set('authorization', `Bearer ${user1Token}`);
...
/********************* GET /user/:userId/movie/:movie_id */

/********************* PATCH /user/:id/movie/userMovieId/update */
...
/********************* POST /users/:id/movie/all */

// describe('POST /users/:id/movie', function() {
// 	test('works for admin', async function() {
// 		const resp = await request(app)
